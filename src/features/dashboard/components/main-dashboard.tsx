...
export function MainDashboard() {
  ...
  // "Ventas por completar": contactos movidos a Alumno con la venta pendiente de registrar.
  const [pendingSales, setPendingSales] = useState<PendingSaleRow[]>([])
  const [salePrefill, setSalePrefill] = useState<React.ComponentProps<typeof RegistrarVentaModal>["prefill"]>(undefined)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const [
        ...
        // Ventas por completar (no depende del periodo: los pendientes siempre importan)
        supabase
          .from("contacts")
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 pb-24">
      ...
      {/* VENTAS POR COMPLETAR (movidos a Alumno a mano, venta pendiente de registrar) */}
      {pendingSales.length > 0 && (
        <section className="space-y-3">
          ...
          <div className="rounded-md border border-green-500/30 bg-green-500/[0.03]">
              ...
              {pendingSales.map((p) => (
                <div key={p.id} className="flex items-center gap-3 px-3 py-2">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-sm border border-green-500/40 bg-green-500/10 px-2.5 py-1 text-[11px] font-medium text-green-400 hover:bg-green-500/20 transition-colors"
                  >
