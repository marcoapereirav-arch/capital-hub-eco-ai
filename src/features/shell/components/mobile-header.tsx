...
// NotificationsBell vive en <OsTopBar> global (layout principal), no aquí
...
export function MobileHeader({ userEmail, userName }: MobileHeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-14 shrink-0 items-center justify-between border-b border-border bg-card/95 px-4 pt-safe backdrop-blur-md md:hidden">
      ...
      {/* Espacio reservado a la derecha para que el OsTopBar global (campana)
          no se solape con el avatar. Padding-right del header. */}
      <Link
        href="/dashboard"
        className="tap-target flex items-center justify-center mr-12"
        aria-label="Perfil"
      >
