...
import {
  ...
  getScript,
  updateScript,
} from '@/features/content-intel/services/script-generator'
...
const PatchSchema = z.object({
  user_edited_markdown: z.string().max(20_000).optional(),
...
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    if (!script) return Response.json({ ok: false, error: 'not_found' }, { status: 404 })
...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  try {
    const supabase = createAdminClient()
    const script = await updateScript(supabase, id, parsed.data)
    return Response.json({ ok: true, script })
  } catch (err) {
    return Response.json({ ok: false, error: toErrorMessage(err) }, { status: 500 })
  }
