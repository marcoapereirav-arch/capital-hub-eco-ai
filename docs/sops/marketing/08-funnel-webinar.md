...
- **2026-07-30 (v5) — Rediseño de la landing (feedback Marco, referencia `live.mkthackers.com/mkt-hackers-optin-d`).**
  ...
  - **2026-07-31 · BUG: el formulario se pegó al contador en móvil.** Al sacar el formulario a su propia columna (escritorio), quedó FUERA del elemento donde estaban declaradas `--sep` y `--paso`. Sin ellas, `margin-top: var(--paso)` no resolvía y caía a 0: pegado. **Arreglo: las dos medidas del ritmo se declaran en `.hero-body`, que es el padre de las DOS columnas.** Regla derivada: **una variable CSS que gobierna el ritmo se declara en el ancestro común de todo lo que la usa**; si se reestructura el layout, hay que comprobar que ningún hijo se ha quedado fuera de su alcance.
  - **2026-07-31 · Escritorio: el titular en 3 líneas parejas y columnas equilibradas.** A 44px partía en 4 líneas de largos muy distintos (una sola palabra arriba, otra larguísima debajo): eso era lo que se leía desordenado. Bajado a 40px, que es lo justo para que quepan 3, más `text-wrap: balance` para igualarlas. Y la tarjeta gana altura (más padding, campos de 3.4rem) para que las dos columnas pesen parecido y no quede un hueco enorme debajo del formulario.
  - **2026-07-31 · Escritorio: dos columnas ALINEADAS ARRIBA y contenido que llena la pantalla.** Restaurar las dos columnas no bastaba, había que dejarlas bien:
    - **Las dos columnas arrancan a la MISMA altura** (`items-start`). Antes iban centradas cada una por su lado y, al tener alturas distintas, la tarjeta quedaba flotando a media página: eso era lo que se veía desalineado.
    - **Tamaños grandes a propósito en escritorio** (titular hasta 54px, cuenta atrás a 38px, campos a 3.25rem). Con tipos pequeños el hero dejaba franjas muertas arriba y abajo; el contenido tiene que llenar la pantalla.
    - **Columna de la derecha de ancho FIJO** (24rem, 25rem en pantallas anchas) y contenedor a `max-w-5xl`: las dos columnas se leen como un solo bloque, no como dos manchas flotando.
    - **El texto se acota a 31rem dentro de su columna** para que el bloque forme un rectángulo limpio en vez de líneas de largos dispares.
  - **2026-07-31 · DOS COLUMNAS EN ESCRITORIO. Restaurado. Y la lección: no cambiar lo que no se pidió.** Pasé el hero a una sola columna por decisión propia, sin que Marco lo pidiera. Sus palabras: *"¿quién carajos te dijo a ti que tienes que pasarlo a una sola columna? Déjalo en dos columnas, como estaba"*. **En escritorio (`min-width: 1024px`) el hero es de DOS COLUMNAS: el texto a la izquierda, el formulario a la derecha. En móvil se apila. Esto no se cambia sin que Marco lo pida.**
    - **REGLA que se deriva y aplica a todo:** cuando Marco pide un ajuste concreto, se toca ESO y nada más. Si de paso creo que otra cosa mejoraría, se propone; no se hace. Es la misma lección que con el color lila y con el copy de "Plazas limitadas": tres veces el mismo fallo en un día.
    - **Y "un poco" quiere decir un poco.** Me pidió acercar *un poco* el bloque del evento a la marca, avisando *"no exageres"*, y los dejé pegados a 7px. Ahora quedan a ~14px en móvil: más cerca que los 24px de partida, pero siguen siendo dos bloques distintos y se ven como tales.
