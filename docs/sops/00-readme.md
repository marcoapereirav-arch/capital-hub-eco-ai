...
- **2026-08-29**: **Historial diario de la actividad del setter.** El parte diario dejaba de existir en cuanto se corregía: el valor viejo se pisaba, no se guardaba quién lo escribió ni quién lo corrigió, y no había ninguna pantalla donde ver el día a día. Ahora la base guarda **una línea por cada guardado** (con el antes, el después y quién lo firmó), escrita por un disparador para que no se pueda guardar sin dejar rastro, y un administrador puede corregir el parte de un setter quedando firmado. Pantalla nueva `/actividad`, primera ruta de la sección **Ventas** del menú. Nuevo SOP [`producto/63`](producto/63-actividad-setter-historial.md).
- **2026-08-07**: Actualización del sistema NVISION (v5 + skills oficiales). Llega la **puerta de entrada** (`.claude/hooks/puerta-de-entrada.mjs`: no se escribe en la carpeta principal ni sin PRP aprobado) y las órdenes de una sola vez `npm run publicar` / `npm run cerrar`. **Tres piezas de la plantilla no se adaptaban a este proyecto** y quedaron arregladas en local: la puerta llegaba muerta con rutas que tienen un espacio (`Capital Hub`), `publicar` exigía escribir la dirección de la web a mano, y exigía un campo `version` que aquí se llama `sha`. Nuevo SOP [`sistemas/11-plantilla-nvision-no-se-adapta-sola.md`](sistemas/11-plantilla-nvision-no-se-adapta-sola.md), con el prompt para que NVISION lo corrija en origen.
- **2026-07-24**: Formación **IA Integrator** rehecha **en la App** (donde toca). El contenido anterior (3 manuales de Vibe Coding y Git heredados de NVISION) se retira entero por indicación de Marco y se sustituye por sus **tres entrenamientos**: cómo funciona todo, cómo usar el sistema y trabajar en equipo. Las tres guías quedan construidas y publicadas como recursos de la formación en la App (antes había tres guías con el contenido viejo). Nace el **hub de recursos** de la formación (acceso arriba del todo y en la cabecera fija), el volver al sitio exacto, el kit visual único y la skill **`formacion-visual`**. Corregido de paso: el trabajo se empezó por error en el OS pese a la REGLA DE ORO del SOP [`producto/02`](producto/02-arquitectura-os-app.md), y se arregló el `overflow-x` del body que rompía `position: sticky` en toda la App. Ver [`producto/ia-integrator`](producto/ia-integrator/00-readme.md) y [`producto/51`](producto/51-recursos-formacion-app.md).
