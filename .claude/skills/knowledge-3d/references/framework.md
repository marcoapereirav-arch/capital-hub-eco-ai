...
```
Esto identifica las "carpetas raíz inmovibles" del Knowledge. Cada `assistant_sops.quadrant` matchea uno.

**Folders** (jerárquico, en BD `assistant_folders`):
```sql
id uuid PK
name text
...
El trigger SQL `assistant_folders_check_cycle` previene el caso peligroso: mover una folder dentro de su propio descendiente. BFS comprueba ancestros del nuevo parent — si encuentra el id de la folder, raise exception.
